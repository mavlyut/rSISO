...
turbo_decoder::turbo_decoder(short_domain::soft_decoder* dec_row, short_domain::soft_decoder* dec_col)
	...
	, ans(n1 * n2)
	, enc1t(n1, short_domain::get_empty(k2))
...
long_domain::binvector turbo_decoder::encode(long_domain::binvector const& c) {
	fail(c.size() == dim(), "turbo, encode: incorrect dim");
	__log("Turbo.encode: " << c << std::endl);
	for (unsigned i = 0; i < k2; ++i) {
		auto row = dec_row->encode(subvector(c, k1 * i, k1 * (i + 1)));
		for (unsigned j = 0; j < n1; ++j) {
			short_domain::setbit(enc1t[j], i, short_domain::getbit(row, j));
		}
	}
	...
	for (unsigned j = 0; j < n1; ++j) {
		...
		for (unsigned i = 0; i < n2; ++i) {
			setbit(ans, i * n1 + j, short_domain::getbit(row, i));
		}
	}
